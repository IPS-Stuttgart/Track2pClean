...
def _subject_support(result: str = "stable_subject_support") -> dict[str, str]:
    return {
        "status": "complete",
...
def test_bundle_promotes_core_when_central_gate_passes() -> None:
    decision = evaluate_identity_history_bundle(
        _identity_promotion(),
        subject_support=_subject_support(),
    )
    ...
    assert decision["subject_support"]["result"] == "stable_subject_support"
...
def test_bundle_reports_missing_subject_support_as_incomplete() -> None:
    decision = evaluate_identity_history_bundle(_identity_promotion())

    assert decision["status"] == "incomplete"
    assert decision["paper_row"] == ""
    assert decision["subject_support"]["status"] == "not_evaluated"
    assert "subject-support" in decision["recommendation"]
...
def test_bundle_rejects_promotable_status_with_weak_subject_support() -> None:
    decision = evaluate_identity_history_bundle(
        _identity_promotion(),
        subject_support=_subject_support("weak_subject_support"),
    )

    assert decision["status"] == "not_promotable_core_method"
    assert decision["paper_row"] == ""
    assert decision["core_evidence_result"] == "inconsistent_core_evidence"
    assert decision["subject_support"]["status"] == "exploratory"
...
def test_bundle_markdown_names_guardrail() -> None:
    ...
    assert "Subject support" in markdown
