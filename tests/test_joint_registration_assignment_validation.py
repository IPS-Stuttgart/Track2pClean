...
@pytest.mark.parametrize("bad_relief", [True, np.bool_(False), np.nan, np.inf, -np.inf, -0.1])
...
@pytest.mark.parametrize("bad_tolerance", [True, np.bool_(False), np.nan, np.inf, -np.inf, -1.0])
...
@pytest.mark.parametrize("bad_quantile", [True, np.bool_(False), np.nan, np.inf, -0.01, 1.01])
...
def test_high_confidence_anchor_edges_rejects_malformed_min_anchor_edges(bad_min_anchor_edges):
...
@pytest.mark.parametrize("bad_margin", [True, np.bool_(False), np.nan, np.inf, -np.inf, -0.1])
