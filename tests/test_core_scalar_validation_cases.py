...
def test_position_covariances_accepts_zero_dimensional_regularization_array() -> None:
    covariances = _single_roi_plane().position_covariances(regularization=np.array(1.0e-6))
...
def test_constant_velocity_state_moments_rejects_vector_velocity_variance() -> None:
    with pytest.raises(ValueError, match="velocity_variance must be a finite non-negative value"):
        _single_roi_plane().to_constant_velocity_state_moments(velocity_variance=np.array([25.0]))
