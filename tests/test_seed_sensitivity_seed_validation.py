...
def test_seed_sensitivity_seed_resolver_rejects_silent_index_coercions(seed_sessions) -> None:
...
def test_seed_sensitivity_seed_resolver_rejects_invalid_session_count(n_sessions) -> None:
...
def test_seed_sensitivity_seed_resolver_accepts_numpy_integer_sequence() -> None:
    assert audit._resolved_seed_sessions(np.asarray([0, 2], dtype=np.int64), n_sessions=3) == (0, 2)
