...
def test_full_mht_conflict_demo_is_registered():
    canonical = cli._BENCHMARK_ALIASES["track2p-full-mht-conflict-demo"]

    assert canonical == "track2p-policy-full-mht-conflict-demo"
    assert (
        cli._BENCHMARK_ALIASES["track2p-pyrecest-full-mht-conflict-demo"]
        == canonical
    )
    assert cli._BENCHMARK_COMMANDS[canonical].module == (
