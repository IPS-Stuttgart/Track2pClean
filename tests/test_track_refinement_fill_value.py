...
def _geometry_issue(
    *, track_index: object = 0, session_index: object = 1
) -> TrackGeometryIssue:
...
@pytest.mark.parametrize(
    "bad_track_index", [True, np.bool_(False), "0", b"0", 0.5, np.nan]
)
...
@pytest.mark.parametrize(
    "bad_session_index", [True, np.bool_(False), "1", b"1", 1.5, np.nan]
)
def test_split_tracks_at_issues_rejects_malformed_issue_session_index(
    bad_session_index,
):
    track_rows = np.array([[0, 1, 2]], dtype=int)
...
def test_split_tracks_at_issues_rejects_out_of_bounds_issue_track_index(
    bad_track_index,
):
    track_rows = np.array([[0, 1, 2]], dtype=int)
...
def test_split_tracks_at_issues_rejects_out_of_bounds_issue_session_index(
    bad_session_index,
):
    track_rows = np.array([[0, 1, 2]], dtype=int)
