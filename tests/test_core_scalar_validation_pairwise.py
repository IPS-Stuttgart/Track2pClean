...
def test_pairwise_cost_rejects_nonfinite_large_cost() -> None:
    ...
    with pytest.raises(ValueError, match="large_cost must be a finite strictly positive value"):
...
def test_pairwise_cost_rejects_zero_centroid_scale() -> None:
    ...
    with pytest.raises(ValueError, match="centroid_scale must be a finite strictly positive value"):
