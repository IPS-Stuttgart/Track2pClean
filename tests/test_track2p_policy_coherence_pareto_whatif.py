...
def test_remove_edge_splits_component_and_scores_exact_delta() -> None:
    ...
    baseline = score_track_matrices(predicted, reference)

    simulation = audit._simulate_remove_edge(predicted, (1, 2, 2, 99))
    candidate = score_track_matrices(simulation.candidate, reference)
    delta = audit._score_deltas(baseline, candidate)

    assert simulation.applied
    assert simulation.candidate.tolist() == [[1, 2, -1, -1], [-1, -1, 99, 4]]
    assert delta["pairwise_false_positives"] == -1
    assert delta["complete_track_false_positives"] == -1
...
def test_add_edge_inserts_clean_missing_target() -> None:
    ...
    assert delta["pairwise_true_positives"] == 1
