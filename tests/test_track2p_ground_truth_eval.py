...
def test_complete_track_score_ignores_rows_with_missing_roi_entries():
    ground_truth = TrackTable(
    ...
    prediction = TrackTable(
        session_names=("s1", "s2", "s3"),
        tracks=np.array([[1, 2, 3], [4, -1, 6], [7, 8, 9]], dtype=int),
    )

    evaluation = evaluate_track_table_prediction(ground_truth, prediction)

    assert evaluation.n_exact_full_track_matches == 1
    # Only complete rows contribute to CT: TP=1, FP=1, FN=0.  The matching
    # incomplete row [4, -1, 6] must not inflate the complete-track score.
    assert evaluation.complete_tracks == 2 / 3
