...
def install_fov_affine_warp_validation() -> None:
    ...
    if not _has_marker_in_wrapper_chain(original_image_warp, _IMAGE_PATCH_MARKER):
    ...
    if not _has_marker_in_wrapper_chain(original_mask_warp, _MASK_PATCH_MARKER):
...
def _patch_estimate_controls(module: Any) -> None:
    ...
    if _has_marker_in_wrapper_chain(original_estimate, _ESTIMATE_PATCH_MARKER):
        return
...
def _has_marker_in_wrapper_chain(function: Any, marker: str) -> bool:
    seen: set[int] = set()
    current = function
