...
def install_fov_affine_warp_validation() -> None:
    ...
    if not _function_chain_has_patch(original_image_warp, _IMAGE_PATCH_MARKER):
    ...
    if not _function_chain_has_patch(original_mask_warp, _MASK_PATCH_MARKER):
...
def _patch_estimate_controls(module: Any) -> None:
    ...
    if _function_chain_has_patch(original_estimate, _ESTIMATE_PATCH_MARKER):
        return
