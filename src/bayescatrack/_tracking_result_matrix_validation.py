    ...
    if not getattr(original_post_init, _PATCH_MARKER, False):
        ...
        def subject_tracking_result_post_init_with_matrix_validation(self: Any) -> Any:
            ...
            track_rows = _normalize_track_rows(
                getattr(self, "track_rows"), fill_value=fill_value
            )
        ...
        _mark_patch(
            subject_tracking_result_post_init_with_matrix_validation, original_post_init
        )
...
def _patch_track_rows_first_arg(module: Any, name: str) -> None:
    ...
    def function_with_track_row_validation(
        track_rows: Any, *args: Any, **kwargs: Any
    ) -> Any:
        ...
        return original(
            _normalize_track_rows(track_rows, fill_value=fill_value), *args, **kwargs
        )
...
def _patch_track_rows_third_arg(module: Any, name: str) -> None:
    ...
    @wraps(original)
    def function_with_track_row_validation(
        first: Any, second: Any, track_rows: Any, *args: Any, **kwargs: Any
    ) -> Any:
        if "fill_value" not in kwargs:
...
def _normalize_link_target_indices(
    values: Any, *, expected_shape: tuple[int, int], session_count: int, fill_value: int
) -> np.ndarray:
    array = np.asarray(values, dtype=object)
...
def _normalize_roi_or_fill_value(
    value: Any, *, field_name: str, fill_value: int
) -> int:
