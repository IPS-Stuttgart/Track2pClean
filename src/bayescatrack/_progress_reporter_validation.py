"""Strict validation for benchmark progress-reporter controls.

``ProgressReporter`` writes status information to stderr only when explicitly
enabled.  Using Python truthiness here makes string-like values such as
``"false"`` enable progress output instead of being rejected, which is surprising
for programmatic benchmark configuration and manifest-driven runs.
"""
...
def install_progress_reporter_validation() -> None:
    """Install idempotent strict validation for ``ProgressReporter.enabled``."""
