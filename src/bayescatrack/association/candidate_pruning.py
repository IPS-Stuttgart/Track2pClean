"""Candidate pruning helpers for calibrated multi-session assignment.

The global assignment solver is most robust when the candidate graph is high
recall but not fully dense.  This module provides deterministic row/column top-k
and probability/cost threshold pruning while preserving the rectangular matrix
interface expected by PyRecEst.  Pruned entries are replaced by a large finite
cost so downstream thresholding can discard them without changing matrix shape.
"""
...
class CandidatePruningConfig:
    """Configuration for cost/probability candidate pruning.

    Parameters are unioned: a candidate is kept when it is selected by any of the
    enabled row/column top-k or threshold criteria.  If no criterion is enabled,
    all finite costs are kept.
    """
...
def candidate_pruning_config_from_mapping(
    ...
    """Normalize optional pruning config values."""
...
def candidate_mask_from_costs(
    ...
    *,
    ...
    """Return a boolean candidate mask for a pairwise assignment matrix."""
