...
def _patch_session_edge_validator() -> None:
    original = _multi_hypothesis._validated_session_edge  # pylint: disable=protected-access
    ...
    def validated_session_edge(edge: Any, *args: Any, **kwargs: Any) -> Any:
        if isinstance(edge, _BYTES_LIKE_SEQUENCE_TYPES):
            raise ValueError(f"{kwargs.get('name', 'edge')} must be a two-item session edge")
    ...
    _multi_hypothesis._validated_session_edge = validated_session_edge  # pylint: disable=protected-access


def _patch_edge_candidate_validator() -> None:
    original = _multi_hypothesis._validated_edge_candidate  # pylint: disable=protected-access
    if getattr(original, _PATCH_MARKER, False):
        return
    ...
    def validated_edge_candidate(candidate: Any, *args: Any, **kwargs: Any) -> Any:
        if isinstance(candidate, _BYTES_LIKE_SEQUENCE_TYPES):
            raise ValueError(f"{kwargs.get('name', 'candidate')} must be a three-item edge candidate")
    ...
    _multi_hypothesis._validated_edge_candidate = validated_edge_candidate  # pylint: disable=protected-access


def _patch_consensus_edge_validator() -> None:
    original = _multi_hypothesis._normalize_consensus_edge  # pylint: disable=protected-access
    if getattr(original, _PATCH_MARKER, False):
        return
    ...
    def normalize_consensus_edge(edge: Any, *args: Any, **kwargs: Any) -> Any:
        if isinstance(edge, _BYTES_LIKE_SEQUENCE_TYPES):
            raise ValueError(f"{kwargs.get('name', 'edge')} must be a four-item consensus edge")
        return original(edge, *args, **kwargs)

    setattr(normalize_consensus_edge, _PATCH_MARKER, True)
    setattr(normalize_consensus_edge, "_bayescatrack_original", original)
    _multi_hypothesis._normalize_consensus_edge = normalize_consensus_edge  # pylint: disable=protected-access
