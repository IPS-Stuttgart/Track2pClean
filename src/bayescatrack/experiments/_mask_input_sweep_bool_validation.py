"""Strict validation for mask-input sweep boolean option sequences."""
...
_PATCH_ATTR = "_track2pclean_mask_input_sweep_bool_validation"


def install_mask_input_sweep_bool_validation() -> None:
    """Install idempotent fail-fast validation for public sweep option tuples."""

    from . import track2p_mask_input_sweep as sweep

    original_normalise_bool_options = sweep._normalise_bool_options  # pylint: disable=protected-access
    if getattr(original_normalise_bool_options, _PATCH_ATTR, False):
        return
    ...
    setattr(_normalise_bool_options, "_track2pclean_original", original_normalise_bool_options)
    sweep._normalise_bool_options = _normalise_bool_options  # pylint: disable=protected-access


__all__ = ["install_mask_input_sweep_bool_validation"]
