"""Decision helper for FullMHT history-dynamics probe manifests.

The history-dynamics objective asks whether terminal MHT hypotheses contain an
edge that is an outlier relative to the rest of the same identity history.  This
module keeps the interpretation frozen: a candidate is interesting only if it
improves complete-track F1 micro without any reported pairwise/complete-track
micro or macro F1 loss across more than one nearby motion-history weight.
"""
...
class HistoryDynamicsDecisionConfig:
    """Frozen row names and tolerance for history-dynamics probes."""
...
def evaluate_history_dynamics_decision(
    rows: Sequence[Mapping[str, Any]],
    *,
    config: HistoryDynamicsDecisionConfig | None = None,
) -> dict[str, Any]:
    """Evaluate whether history-dynamics weighting is stable enough to promote."""

    cfg = config or HistoryDynamicsDecisionConfig()
    ...
    if missing:
        return {
            ...
            "recommendation": "rerun history-dynamics probe manifest",
        }
    ...
    result = _history_dynamics_result(
        n_viable=len(viable),
        n_ties=len(ties),
    ...
    decision: dict[str, Any] = {
        ...
        "history_dynamics_result": result,
...
def format_decision_markdown(decision: Mapping[str, Any]) -> str:
    """Format a compact history-dynamics decision note."""
    ...
    lines = [
        "# FullMHT History Dynamics Decision",
        "",
        f"Result: `{decision['history_dynamics_result']}`",
...
def _history_dynamics_result(
    *,
    ...
    if n_pairwise_regressions > 0:
        return "history_dynamics_pairwise_regression"
    if n_complete_regressions > 0:
        return "history_dynamics_complete_regression"
    if n_viable >= 2:
        return "history_dynamics_stable_gain"
    if n_viable == 1:
        return "history_dynamics_single_weight_gain"
    if n_ties == n_candidates:
        return "history_dynamics_ties_baseline"
    return "history_dynamics_not_promotable"
...
def _recommendation(result: str) -> str:
    ...
    if result == "history_dynamics_ties_baseline":
        return "record as history-dynamics validation but do not promote as better"
    return "do not promote history dynamics from this probe"


def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m bayescatrack.experiments.full_mht_history_dynamics_decision",
        description="Summarize history-dynamics FullMHT probe comparison rows.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    decision = evaluate_history_dynamics_decision(load_comparison_rows(args.comparison_csv))
