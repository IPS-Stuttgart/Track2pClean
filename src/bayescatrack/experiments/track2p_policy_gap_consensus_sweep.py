...
class GapConsensusSweepConfig:
    ...
    base_component: ComponentCleanupConfig = field(
    ...
    def __post_init__(self) -> None:
        object.__setattr__(
            ...
            _finite_nonnegative_tuple(
                self.base_iou_distance_thresholds, name="base_iou_distance_thresholds"
            ),
        )
        object.__setattr__(
            self,
        ...
        object.__setattr__(
            ...
            _boolean_tuple(
                self.require_complete_track_options,
                name="require_complete_track_options",
            ),
        )
        object.__setattr__(
            self,
        ...
        object.__setattr__(
            self, "max_gaps", _positive_int_tuple(self.max_gaps, name="max_gaps")
        )
        object.__setattr__(
            self, "consensus_modes", _consensus_mode_tuple(self.consensus_modes)
        )
        object.__setattr__(
            self,
            "stability_iou_distance_thresholds",
            _finite_nonnegative_tuple(
                self.stability_iou_distance_thresholds,
                name="stability_iou_distance_thresholds",
            ),
        )
        object.__setattr__(
            self,
            "min_support_fraction",
            _support_fraction_value(
                self.min_support_fraction, name="min_support_fraction"
            ),
        )
...
class GapConsensusSweepOutput:
    ...
    def best_rows(self) -> tuple[dict[str, float | int | str], ...]:
        ...
        return tuple(
            row for row in self.rows if int(row["gap_consensus_sweep_best"]) == 1
        )
...
def run_track2p_policy_gap_consensus_sweep(
    ...
    *,
    ...
    candidate_rows: list[
        tuple[str, ConsensusCleanupConfig, int, list[dict[str, Any]]]
    ] = []
    ...
    ranks = {
        str(row["approach"]): int(row["gap_consensus_sweep_rank"]) for row in ranked
    }
    objectives = {
        str(row["approach"]): float(row["gap_consensus_sweep_objective"])
        for row in ranked
    }
    ...
    return GapConsensusSweepOutput(
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--max-gaps", default=str(TRACK2P_POLICY_GAP_CONSENSUS_DEFAULT_MAX_GAP)
    )
    parser.add_argument("--consensus-modes", default="risk-and-stability")
    parser.add_argument(
        "--objective",
        choices=GAP_CONSENSUS_SWEEP_OBJECTIVES,
...
def main(argv: list[str] | None = None) -> int:
    ...
    sweep_config = GapConsensusSweepConfig(
        base_iou_distance_thresholds=_float_tuple_arg(
            args.base_iou_distance_thresholds, name="base-iou-distance-thresholds"
        ),
        ...
        require_complete_track_options=_bool_tuple_arg(
            args.require_complete_track_options, name="require-complete-track-options"
        ),
        max_splits_per_component=_int_tuple_arg(
            args.max_splits_per_component, name="max-splits-per-component"
        ),
...
def _cleanup_grid(
    config: GapConsensusSweepConfig,
) -> tuple[tuple[int, ConsensusCleanupConfig, int], ...]:
...
def _rank_aggregates(
    ...
    *,
    ...
    enriched = [
        {
            **row,
            "gap_consensus_sweep_objective": _objective_value(row, objective),
            "gap_consensus_sweep_objective_name": objective,
        }
        for row in rows
    ]
    ranked = sorted(
    ...
    return [
        {**row, "gap_consensus_sweep_rank": rank}
        for rank, row in enumerate(ranked, start=1)
    ]


def _objective_value(
    row: Mapping[str, float | int | str], objective: GapConsensusSweepObjective
) -> float:
...
def _annotate_subject_rows(
    candidate_rows: Sequence[
        tuple[str, ConsensusCleanupConfig, int, Sequence[Mapping[str, Any]]]
    ],
    best_candidate: str,
    ...
    for candidate, cleanup, max_gap, rows in candidate_rows:
        for row in rows:
            annotated.append(
                {
                    **dict(row),
                    "gap_consensus_sweep_candidate": candidate,
                    "gap_consensus_sweep_rank": int(ranks[candidate]),
                    "gap_consensus_sweep_best": int(candidate == best_candidate),
                    "gap_consensus_sweep_objective": float(objectives[candidate]),
                    "gap_consensus_sweep_base_iou_distance_threshold": float(
                        cleanup.stability.base_iou_distance_threshold
                    ),
                    "gap_consensus_sweep_split_risk_threshold": float(
                        cleanup.component.split_risk_threshold
                    ),
                    "gap_consensus_sweep_split_penalty": float(
                        cleanup.component.split_penalty
                    ),
                    "gap_consensus_sweep_min_side_observations": int(
                        cleanup.component.min_side_observations
                    ),
                    "gap_consensus_sweep_require_complete_track": int(
                        cleanup.component.require_complete_track
                    ),
                    "gap_consensus_sweep_max_splits_per_component": int(
                        cleanup.max_splits_per_component
                    ),
                    "gap_consensus_sweep_max_gap": int(max_gap),
                    "gap_consensus_sweep_mode": str(cleanup.mode),
                }
            )
    return annotated
