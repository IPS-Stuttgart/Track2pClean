...
def run_oracle_affine_qa_report(config: OracleAffineQAConfig) -> list[dict[str, Any]]:
    ...
    for subject_dir in subject_dirs:
        ...
        reference = _load_reference_for_subject(subject_dir, data_root=config.registration.data, config=benchmark_config)
        ...
        reference_matrix = _reference_matrix(reference, curated_only=config.registration.curated_only)
        for source_index, target_index in session_edge_pairs(len(sessions), max_gap=config.registration.max_gap):
            linked_source_rois = _linked_source_rois(reference_matrix, source_index, target_index)
            ...
            fit = _fit_affine_xy(source_xy, target_xy, ridge=config.ridge, require_full_rank=config.require_full_rank)
            baseline = register_plane_pair(source_session.plane_data, target_session.plane_data, transform_type=config.registration.transform_type)
            oracle = _oracle_affine_registered_plane(source_session.plane_data, target_session.plane_data, fit)
            rows.append(_edge_row(subject_dir.name, source_session.session_name, target_session.session_name, source_index, target_index, links, source_session.plane_data, target_session.plane_data, baseline, oracle, fit, config.registration.transform_type))
    return rows


def summarize_oracle_affine_qa(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
...
def write_oracle_affine_qa_results(rows: Sequence[Mapping[str, Any]], output: Path, fmt: str) -> None:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Compare baseline registration to a manual-GT oracle affine warp."
...
def _manual_gt_links(source_plane: CalciumPlaneData, target_plane: CalciumPlaneData, reference_matrix: np.ndarray, source_index: int, target_index: int) -> list[tuple[int, int, np.ndarray, np.ndarray]]:
    ...
    for track in reference_matrix:
        ...
        if source_roi_int not in source_lookup or target_roi_int not in target_lookup:
            continue
        links.append((source_lookup[source_roi_int], target_lookup[target_roi_int], source_centroids[source_lookup[source_roi_int]], target_centroids[target_lookup[target_roi_int]]))
    return links


def _fit_affine_xy(source_xy: np.ndarray, target_xy: np.ndarray, *, ridge: float = 0.0, require_full_rank: bool = True) -> OracleAffineFit:
    ...
    if ridge > 0.0:
        coef = np.linalg.solve(design.T @ design + ridge * np.eye(3), design.T @ source_xy)
    else:
        coef, _, _, _ = np.linalg.lstsq(design, source_xy, rcond=None)
    residual = design @ coef - source_xy
    return OracleAffineFit(matrix_xy=coef.T, residual_norm=np.linalg.norm(residual, axis=1), rank=rank, condition=condition)


def _oracle_affine_registered_plane(reference_plane: CalciumPlaneData, moving_plane: CalciumPlaneData, fit: OracleAffineFit) -> CalciumPlaneData:
    masks = _warp_masks_by_affine_xy(np.asarray(moving_plane.roi_masks) > 0, fit.matrix_xy, reference_plane.image_shape)
    fov = reference_plane.fov if reference_plane.fov is not None else np.zeros(reference_plane.image_shape, dtype=float)
    ops = {} if moving_plane.ops is None else dict(moving_plane.ops)
    ops.update({"registration_backend": "manual-gt-oracle-affine", "registration_transform_type": "oracle-affine", "oracle_affine_matrix_xy": fit.matrix_xy.tolist()})
    return moving_plane.with_replaced_masks(masks, fov=fov, source=f"{moving_plane.source}_oracle_affine", ops=ops)


def _warp_masks_by_affine_xy(masks: np.ndarray, matrix_xy: np.ndarray, output_shape: tuple[int, int]) -> np.ndarray:
...
def _edge_row(subject: str, source_name: str, target_name: str, source_index: int, target_index: int, links: Sequence[tuple[int, int, np.ndarray, np.ndarray]], source_plane: CalciumPlaneData, raw_target_plane: CalciumPlaneData, baseline_plane: CalciumPlaneData, oracle_plane: CalciumPlaneData, fit: OracleAffineFit, baseline_transform_type: str) -> dict[str, Any]:
    source_locals = np.asarray([link[0] for link in links], dtype=int)
    target_locals = np.asarray([link[1] for link in links], dtype=int)
    baseline_iou = _linked_iou(source_plane.roi_masks[source_locals], baseline_plane.roi_masks[target_locals])
    oracle_iou = _linked_iou(source_plane.roi_masks[source_locals], oracle_plane.roi_masks[target_locals])
    raw_iou = _linked_iou(source_plane.roi_masks[source_locals], raw_target_plane.roi_masks[target_locals]) if source_plane.image_shape == raw_target_plane.image_shape else np.full(len(links), np.nan)
    baseline_distance = _linked_centroid_distance(source_plane, baseline_plane, source_locals, target_locals)
    oracle_distance = _linked_centroid_distance(source_plane, oracle_plane, source_locals, target_locals)
...
def _linked_centroid_distance(source_plane: CalciumPlaneData, target_plane: CalciumPlaneData, source_locals: np.ndarray, target_locals: np.ndarray) -> np.ndarray:
