"""Validate benchmark manifests and print their execution plan without running them."""
...
def build_manifest_plan(manifest: BenchmarkManifest) -> dict[str, Any]:
    """Return a JSON-serializable execution plan for a parsed manifest."""
...
def format_manifest_plan_table(plan: Mapping[str, Any]) -> str:
    """Format a manifest execution plan as Markdown tables."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    """Build the dry-run manifest validation CLI parser."""

    parser = argparse.ArgumentParser(
        prog="bayescatrack benchmark validate-suite",
        description=(
            "Validate a benchmark suite manifest and print the resolved run plan "
