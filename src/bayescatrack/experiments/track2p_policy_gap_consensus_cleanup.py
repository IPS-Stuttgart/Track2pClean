...
The consensus cleanup is intentionally conservative: it removes only risky,
threshold-unstable adjacent bridges.  This module runs that same consensus
splitter on top of the gap-rescue Track2p-policy propagation, so the benchmark
can test the most promising next-step combination: recover isolated misses first,
then prune suspicious false-positive continuations.
"""
...
def run_track2p_policy_gap_consensus_cleanup(
    ...
    *,
    ...
    max_gap: int | None = TRACK2P_POLICY_GAP_CONSENSUS_DEFAULT_MAX_GAP,
    cleanup_config: ConsensusCleanupConfig | None = None,
    apply_splits: bool = True,
) -> ComponentAuditOutput:
    """Run gap-rescue Track2p-policy and split consensus-risky bridges."""
    ...
    for subject_dir in subject_dirs:
        ...
        results.append(
            SubjectBenchmarkResult(
                subject=subject_dir.name,
                variant=(
                    "Track2p-policy gap-rescue consensus bridge cleanup"
                    if apply_splits
                    else "Track2p-policy gap-rescue consensus bridge audit"
                ),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Run Track2p-policy gap rescue, then split bridges that are both "
            "risky and unstable across nearby IoU thresholds."
        ),
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "cleanup. Consecutive links are still preferred."
        ),
    )
    parser.add_argument(
...
def main(argv: list[str] | None = None) -> int:
    """Run the Track2p-policy gap-rescue consensus cleanup CLI."""
