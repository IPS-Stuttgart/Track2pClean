...
def install_aligned_roi_index_validation(reference_module: ModuleType | None = None) -> None:
    ...
    original_roi_indices_for_plane = reference_module._suite2p_roi_indices_for_plane  # pylint: disable=protected-access
    if getattr(original_roi_indices_for_plane, _PATCH_ATTR, False):
        return

    def _suite2p_roi_indices_for_plane_with_shape_validation(plane_data: Any) -> np.ndarray:
        ...
        if roi_indices is not None:
            ...
            if roi_indices_array.ndim != 1:
                raise ValueError("plane_data.roi_indices must be a one-dimensional vector")
